...
if __name__ == '__main__':
    start_time = time()
    target_hosts = ('192.168.80.128', '192.168.80.4', '192.168.80.111')
    ...
    for ip in up_hosts:  # 每循环一次，就扫描一个主机的端口
        print('Scanning %s hosts port, open ports have been found:' % ip, end="")
        for port in target_ports:   # 每循环1次扫描1个端口
            scapy_port(ip, port)
    end_time = time()
    print(waste_time := end_time-start_time)
